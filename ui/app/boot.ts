...
cobudgetApp.run(function (
  ...
  cobudgetApp.announcementsLoaded = announcementsLoadedDeferred.promise;
  ...
  $rootScope.$on("auth:validation-success", function (ev, user) {
    ...
    if (user.is_super_admin) {
      ...
      if (pathComponents[1] === "groups") {
        ...
        Records.memberships
          ...
          .then((data) => membershipsLoadedDeferred.resolve(data));
    ...
    Records.announcements
      .fetch({})
      .then((data) => announcementsLoadedDeferred.resolve(data));
    if (typeof Sentry !== "undefined" && Sentry !== null) {
      Sentry.setUser({ email });
    }
    return __guard__(
      typeof HS !== "undefined" && HS !== null ? HS.beacon : undefined,
      (x) =>
        x.ready(() =>
          HS.beacon.identify({
            name: user.name,
