class GroupAnalyticsService
  attr_reader :group
  attr_accessor :group_bucket_ids, :confirmed_group_member_ids
  ...
  private
    ...
    def contribution_amounts_per_day
      data = Contribution.where(bucket_id: group_bucket_ids).group_by_day(:created_at).sum(:amount)
      {
        x: data.keys.map { |day| day.strftime('%Y-%m-%d') },
        y: data.values
      }
    ...
    def allocation_amounts_per_day
      data = group.allocations.group_by_day(:created_at).sum(:amount)
      {
        x: data.keys.map { |day| day.strftime('%Y-%m-%d') },
        y: data.values
      }
    ...
    def comment_counts_per_day
      data = Comment.where(bucket_id: group_bucket_ids).group_by_day(:created_at).count
      {
        x: data.keys.map { |day| day.strftime('%Y-%m-%d') },
        y: data.values
      }
