class UserService
  ...
  def self.collated_activity_for_group(group)
    [group, {
      ...
      other_buckets_fully_funded: other_buckets_fully_funded.where(group: group)
    }]
  end
  ...
  def self.other_buckets_fully_funded
    @@other_buckets_fully_funded ||= user_group_buckets.where(status: 'funded', funded_at: time_range)
