module.exports =
  ...
  controller: (CurrentUser, Error, LoadBar, $location, Records, $scope, $stateParams, Toast, UserCan, $window) ->
    ...
    $scope.userCanManageBucket = ->
      $scope.bucket && !$scope.bucket.paidAt && $scope.bucket.status != 'refunded' && !($scope.bucket.archivedAt && $scope.bucket.status == 'draft') && ($scope.membership.isAdmin || $scope.bucket.author().id == $scope.membership.member().id)
