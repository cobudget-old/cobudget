...
controller = ($rootScope, $scope, Group) ->
  $rootScope.$watch 'currentBudget', (group) ->
    return unless group

    console.log("current group id", group.current_round_id)
    Round.get(group.current_round_id).then (round) ->
    ...
    # Group.getBudgetBuckets(group.id).then (buckets) ->
