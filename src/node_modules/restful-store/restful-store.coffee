...
require('./capture-stack-trace')

createHttpError = require('http-errors')

### @ngInject ###
module.exports = ($q, $http, config) ->
  class RestfulStore
    constructor: (opts = {}) ->
    ...
    errorHandler: (httpErr) ->
      debug("errorHandler", httpErr)
      err = createHttpError(httpErr.status || 503)
      err.data = recase.camelCopy(httpErr.data)
      $q.reject(err)

    all: (params) ->
      path = @path(params, "all")
      params = recase.snakeCopy(params)
