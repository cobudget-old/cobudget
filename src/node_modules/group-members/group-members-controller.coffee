_ = require('lodash')
debug = require('debug')("group-members")

### @ngInject ###
module.exports = ($scope, $modal, AuthService, group, MembershipModel, MembershipsStore) ->

  $scope.currentUser = AuthService.getCurrentUser()
  $scope.isAdmin = group.isAdmin($scope.currentUser)
  memberships = group.memberships

  # alerts
  $scope.alerts = []

  $scope.addAlert = (alert) ->
    $scope.alerts.push(alert)

  $scope.closeAlert = (index) ->
    $scope.alerts.splice(index, 1)
  # /alerts

  $scope.isCurrentUser = (membership) ->
    $scope.currentUser.getId() == membership.member.getId()

  $scope.openAddMembersModal = ->
    $modal.open(
      require('./add-members-modal.coffee')(
        group: group
      )
    ).result.then (modalRet) ->
      debug("new members modal return", modalRet)

      memberships.add(modalRet.memberships)

      if modalRet.members.length > 0
        $scope.addAlert(
          type: "success"
          msg: "Successfully added #{ modalRet.members.length } new members:
            #{ (_.map modalRet.members, (m) -> m.name).join(", ") }"
        )

      if (modalRet.erroredMembers.length > 0)
        $scope.addAlert(
          type: "danger"
          msg: "Errored while adding #{ modalRet.erroredMembers.length } members:
            #{ (_.map modalRet.erroredMembers, (m) -> m.name).join(", ") }"
        )

      if modalRet.ignoredMembers.length > 0
        $scope.addAlert(
          type: "info"
          msg: "Ignored adding #{ modalRet.ignoredMembers.length } existing members:
            #{ (_.map modalRet.ignoredMembers, (m) -> m.name).join(", ") }"
        )

    , (err) ->
      if (err and err != "backdrop click" and err != "escape key press")
        # handle error
        $modal.open(
          require('error-modal')({ error: err })
        )

  $scope.toggleAdmin = (membership, index) ->
    # toggle admin property
    membership.isAdmin = !membership.isAdmin
    # re-save
    MembershipsStore.save(membership).then null
    , (err) ->
      # re-toggle
      membership.isAdmin = !membership.isAdmin
      # handle error
      $modal.open(
        require('error-modal')({ error: err })
      )

  $scope.removeMember = (membership, index) ->
    MembershipsStore.remove(membership).then ->
      memberships.remove(membership)
