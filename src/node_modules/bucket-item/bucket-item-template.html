<a ui-sref="layout.round.buckets.details({ groupId: group.id, bucketId: bucket.id })">
  <div class="bucket item" ng-class="{ selected: selected }">
    <div class="contribution">
      <input contribution-status="round.balanceStatus" type="number" min="0" max="{{ round.balanceStatus !== 'success' ? (round.myAllocationAmount ? myAllocationAmount : '') : model.myContributionAmount }}" ng-model='model.myContributionAmount' placeholder="0" ng-blur='saveContribution(bucket.myContribution)' ng-readonly="model.readonly" class="{{ model.readonly ? 'inactive' : 'active' }}">
    </div>
