<a ui-sref="layout.round.buckets.details({ groupId: group.id, bucketId: bucket.id })">
  <div class="bucket item" ng-class="{ selected: selected }">
    <div class="contribution" ng-class="{ inactive: !model.readonly, contributing: model.contributing }">
      <span ng-if="!model.contributing" class="action" ng-click="model.startContributing($event)" contribution-status="round.balanceStatus" >Fund</span>
      <div ng-if="model.contributing" class="input-group" contribution-status="round.balanceStatus">
        <span class="currency">$</span>
        <input type="number" min="0" max="{{ round.balanceStatus !== 'success' ? (round.myAllocationAmount ? myAllocationAmount : '') : model.myContributionAmount }}" ng-model='model.myContributionAmount' placeholder="0" ng-blur='saveContribution(bucket.myContribution)' ng-readonly="model.readonly" class="input">
      </div>
    </div>
