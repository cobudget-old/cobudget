<a ui-sref="layout.round.buckets.details({ groupId: group.id, bucketId: bucket.id })">
  <div class="bucket">
    ...
    <input contribution-status="round.balanceStatus" type="number" min="0" max="{{ round.balanceStatus !== 'complete' ? (round.myAllocationAmountCents ? myAllocationAmount : '') : model.myContributionAmount }}" ng-model='model.myContributionAmount' placeholder="0" ng-blur='saveContribution(bucket.myContribution)' >
    </div>
