...
module.exports = (RestangularProvider, config) ->
  ...
  RestangularProvider.setResponseInterceptor (data, operation, what, url, response, deferred) ->
    console.log(data)
    dataCopy = recase.camelCopy(data.plain)
    console.log(data, dataCopy, operation)
    if operation is "get" or operation is "post"
      ...
      return dataCopy[what.substring(0, what.length-1)]
    if operation is "getList"
      return dataCopy[what]

  RestangularProvider.setRequestInterceptor (element, operation, what, url) ->
    elementCopy = recase.snakeCopy(element)
    console.log(elementCopy, operation)
    if operation is "post"
      # Add singular root node to element
      newElem = {}
      newElem[what.substring(0, what.length-1)] = elementCopy
      return newElem
