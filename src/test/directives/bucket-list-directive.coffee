...
describe "bucket list directive", ->
  describe 'load buckets', ->
    ...
    it 'does nothing if rootScope.currentBudget is empty', ->
      ...
      expect(Group.getBudgetBuckets).to.not.have.been.called

    it 'loads buckets if currentBudget exists', ->
      Group.getBudgetBuckets.returns promise.with [{id: 1, my_allocation_total: 100, name: 'bucket', percentage_funded: 20}, {id: 2, my_allocation_total: 100, name: 'bucket 2', percentage_funded: 20}]
      $rootScope.watch_callback {id: 3}
      expect(Group.getBudgetBuckets).to.have.been.calledWith(3)
