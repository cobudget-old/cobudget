class AddStatusAccountToGroup < ActiveRecord::Migration
  def change
    ...
    Group.find_each do |group|
      ...
      if group_user 
        group_membership = Membership.find_by(group_id: group.id, member_id: group_user.id)
        if group_membership
          Account.find(group_membership.status_account_id).change_account_to(account.id)
